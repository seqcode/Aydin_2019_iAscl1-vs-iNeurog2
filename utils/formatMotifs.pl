...
for($i=0; $i<=$#linesCurr && $i<5; $i++) {
	...
	if($#curr>=0){
		if($curr[0] eq "AlignACE"){
			...
			$alignace=1; 
			$i=5;
		}elsif($curr[0] eq "MEME" || $curr[1] eq "DREME"){
			$motif_finder=1;
			$meme=1;
			$i=5;
	...
	######################################################################	
		if($#curr>=0 && ($curr[0] eq "DE" || $curr[0] eq "NA" || $curr[0] eq "PO" || $curr[0] eq "P0" || $curr[0] eq "AC")){#TRANSFAC Format
			...
			if($tmp[0] eq "A" && $#tmp>1){###JASPAR format
				...
				if($noTag==1 && ($#tmp ==3)){###Raw PSSM format (Lx4 - similar to TRANSFAC)
					$finished=0;				
					$x=0;
					$i++;
					while($i<=$#linesCurr && $finished==0){
                ...
                }elsif($noTag==1 && ($#tmp >3)){###Assume PSSM format (4xL similar to Jaspar without letters)
                    $base_cols=0;
                    for($x=0; $x<4; $x++){
                        $i++;
                        $linesCurr[$i] =~ tr/\[\]//d;
                        @tmp = split(" ",  $linesCurr[$i]);
                        if($base_cols==0){$base_cols++;
                            for($z=0; $z<=$#tmp; $z++){
                                $currA[$z]= $tmp[$z];if($currA[$z]<0){$x=4;}
                            }
                        }elsif($base_cols==1){
