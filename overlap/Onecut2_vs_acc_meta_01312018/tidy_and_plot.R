...
Onecut2_iAscl1_iNgn2_shared <- read.table("Onecut2_ia_in_shared_2kb.metaprofile", header=TRUE)
Onecut2_iAscl1_48hr_gt_Onecut2_iNgn2 <- read.table("Onecut2_ia_gt_in_2kb.metaprofile", header=TRUE)
Onecut2_iNgn2_48hr_gt_Onecut2_iAscl1 <- read.table("Onecut2_in_gt_ia_2kb.metaprofile", header=TRUE)
...
dat = tbl_df(Onecut2_iAscl1_iNgn2_shared)
...
d = gather(dat,"class","value",2:6)
#d = d %>% filter(offset>=-100 & offset <=100)
ggplot(d,aes(x=offset,y=value,color=class))+geom_line(size=1.2)+scale_color_manual(values=c("oh"=EB,"ia12h"=iA12h,"ia48h"=iA48h,"in12h"=iN12h,"in48h"=iN48h))+scale_x_continuous(breaks = c(-100,0,100),labels = c("-1kb","0","1kb"))+xlab("Distance from TF binding site")+ylab("Normalized read density")+ggtitle("Onecut2(iA) > Onecut2(iN) at Dox+48h")
ggsave("Onecut2_ia_gt_in_2kb.png")

dat = tbl_df(Onecut2_iNgn2_48hr_gt_Onecut2_iAscl1)
...
d = gather(dat,"class","value",2:6)
#d = d %>% filter(offset>=-100 & offset <=100)
ggplot(d,aes(x=offset,y=value,color=class))+geom_line(size=1.2)+scale_color_manual(values=c("oh"=EB,"ia12h"=iA12h,"ia48h"=iA48h,"in12h"=iN12h,"in48h"=iN48h))+scale_x_continuous(breaks =c(-100,0,100),labels = c("-1kb","0","1kb"))+xlab("Distance from TF binding site")+ylab("Normalized read density")+ggtitle("Onecut2(iN) > Onecut2(iA) at Dox+48h")
ggsave("Onecut2_in_gt_ia_2kb.png")
