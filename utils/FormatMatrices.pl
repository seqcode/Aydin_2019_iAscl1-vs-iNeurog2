...
my $trimming=0;
my $Motif_nametag = "Motif";
if($#ARGV>1){
	$evalCutoff = $ARGV[2];
}

#if($#ARGV>1){
#	$Motif_nametag = $ARGV[2];
#}
if($#ARGV>2){
	$trimming = 1;
	$minIC = $ARGV[3];
...
for($i=0; $i<=$#linesCurr; $i++) {
	...
	if($motif_finder==1){
		if($bioprospector==1 && $#curr>=0 && ($curr[0] eq "Blk1" || $curr[0] eq "Blk2")){#BioProspector results file
			...
			$WeightOrFam ="";
			if($#curr>8){
				$eval = $curr[9] ## Storing the evalue
			}
		...
		}elsif($annspec==1 && $curr[0] eq "ALR" ){#ANN-Spec results file
			$tmpNum = $numMotifs+1;
			$InfName = $Motif_nametag.$tmpNum;
			$WeightOrFam ="";
			$i++;$finished=0;$x=0;			
	...
	######################################################################	
		if($#curr>=0 && ($curr[0] eq "DE" || $curr[0] eq "NA" || $curr[0] eq "PO" || $curr[0] eq "P0")){#TRANSFAC Format
			$currTag = $curr[0];
			if($currTag eq "DE" || $currTag eq "NA"){
				@name = split(/\s+/, $curr[1]);
				$InfName = $name[0];
				$WeightOrFam ="";
				if($#curr>1 && $curr[2] ne ""){
	...
	if($good==1){
		...
		print OUT "DE\t$InfName\t$WeightOrFam\n";
		for($j=0; $j<=$#currA; $j++) {
...
sub column2consensus{
	...
	@two_base_c =();	
	$two_base_c[0]=$f[1]+$f[3];
	$two_base_c[1]=$f[0]+$f[2];
	$two_base_c[2]=$f[0]+$f[3];
	$two_base_c[3]=$f[1]+$f[2];
	$two_base_c[4]=$f[2]+$f[3];
	$two_base_c[5]=$f[0]+$f[1];
